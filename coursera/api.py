...
from .define import (  # New feature, Notebook (Python Jupyter)
    IN_MEMORY_EXTENSION,
    IN_MEMORY_MARKER,
    ...
    def _get_notebook_folder(self, url, jupyterId, **kwargs):
        ...
        for content in reply["content"]:
            ...
            if content["type"] == "directory":
                ...
                r = requests.get(
                    tmp_url.replace(" ", "%20"), cookies=self._session.cookies
                )
                if not os.path.exists(
                    self._course_name + "/notebook/" + head + "/" + tail
                ):
                    logging.info("Downloading %s into %s", tail, head)
                    with open(
                        self._course_name + "/notebook/" + head + "/" + tail, "wb+"
                    ) as f:
                        f.write(r.content)
                else:
            ...
            if subtitles is not None:
                ...
                for language_with_alts in requested_subtitle_list:
                    ...
                    grouped_language_list = [
                        l.strip() for l in language_with_alts.split("|")
                    ]
